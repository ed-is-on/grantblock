 ...
 */
 ...
 async function addValidatingGrantees(validatorParams){
    ...
    // var arID = validatorParams.request.getIdentifier().toString();

    // const arRegistry = await getAssetRegistry(NS + '.ActionRequest');
    // var currentReq = arRegistry.get(arID);

    // const gRegistry =  await getParticipantRegistry(NS + '.Grantee');
    // var granteeObjectList = await gRegistry.getAll();
    // var granteeIdList = granteeObjectList.map(function(grantee){return grantee['granteeId'];});

    // //TODO - CHECK 1 - Does the record already have validators? If so, which ones?
    
    // // CHECK 2 - Are there enough grantees for the number of validators requested?
    // if(numValidators-1 > granteeIdList.length){
    //     throw new Error("Not enough grantees for the number of validators requested!");
    // }
    // // Enter the loop with number of validators left to add
    // var vToAdd = numValidators;
    
    // while(vToAdd > 0){
    // // Pick a number from the list and find the corresponding grantee
    //     var randomInt = Math.floor(Math.random()*(granteeIdList.length));
    //     var selectedG = await gRegistry.get(granteeIdList[randomInt]);
    // // CHECK 3 - Is the selected grantee the owner of currentReq?
    //     // var CHECK3 = false;
    //     // if(!(selectedG === currentReq.owner)){
    //     //     CHECK3 = true;
    //     // }
    // // CHECK 4 - Is that grantee in the assignedValidators for the currentReq already?
    //     // var CHECK4 = true;

    //     //var assignedVList = currentReq.assignedValidators.map(function(grantee){return grantee['granteeId'];});
        
    //     // if(currentReq.assignedValidators){
    //     //     var aVListLength = currentReq.assignedValidators.length;
    //     // }else{
    //     //     var aVListLength = 0;
    //     // }

    //     // var i;
    //     // for (i = 0; i < aVListLength; i++) {
    //     //     if (currentReq.assignedValidators[i] === selectedG) {
    //     //         CHECK4 = false;
    //     //     }
    //     // }

    // // If both CHECKS 3 & 4 succeed, then add the grantee to the list of assigned validators & decrement vToAdd
    //     // if(CHECK3===true && CHECK4===true){
    //         // selectedG = await gRegistry.get(selectedG.granteeId);
    //         // currentReq = await arRegistry.get(currentReq.requestId);
    //         currentReq.assignedValidators.push(selectedG);
    //         vToAdd=vToAdd-1;
    //     // }
    // }
    ...
    var granteesCount = numGrantees.length;
    ...
    var identifierArray = [];
    var newValue;
    identifierArray.push(currentReq.owner.granteeId);

    while(validatorCount>0){
        //select a random number
        var validNum = Math.floor(Math.random()*granteesCount);
        //take the value of that random number from the list of grantee IDs
        var granteeidentifier = numGrantees[validNum];
        //create an array to hold a list of the identifiers we've added in to avoid repeats
        
        // var x;
        // for(x=0; x < identifierArray.length; x++){
        //     if(identifierArray[x]===granteeidentifier){
        //         newValue = false;
        //     }
        // }

      	// identifierArray.forEach(function(identifierArray[x]){
        // 	if(granteeidentifier === identifierArray[x]){
        //         newValue = false;
        //     }
        // });
      	...
      	//if(newValue === true){
            ...
            // identifierArray.push(granteeidentifier);
            validatorCount=validatorCount-1;
        //}

        //check to make sure that the ID you selected does not match the ID of the req owner
        // if(!(granteeidentifier===currentReq.owner.granteeId)){
        //     //grab the participant associated with that record from the registry
        //     var pushReq = await gRegistry.get(granteeidentifier);

        //     //check that the list of approvers does not already include the one you selected
        //     if(
        //         !(identifierArray.includes(granteeidentifier))
        //     ){
        //         //if it does not, add the record to assigned identifiers, add the ID to the list, and decrement the validator count
        //         currentReq.assignedValidators.push(pushReq);
        //         identifierArray.push(granteeidentifier);
        //         validatorCount=validatorCount-1;
        //     }
